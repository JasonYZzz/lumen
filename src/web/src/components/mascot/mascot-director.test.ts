...
describe('mascot director', () => {
  it('shows one early welcome action so motion is discoverable', () => {
    expect(FIRST_IDLE_ACTION_DELAY_MS).toBe(2_200)
  })

  it('keeps automatic actions inside the 8–18 second window', () => {
    expect(nextIdleActionDelay(() => 0)).toBe(8_000)
    expect(nextIdleActionDelay(() => 1)).toBe(18_000)
  })
  ...
  it('slows blinking down in reduced motion mode', () => {
    expect(nextBlinkDelay('full', () => 0)).toBe(3_000)
    expect(nextBlinkDelay('full', () => 1)).toBe(7_000)
    expect(nextBlinkDelay('reduced', () => 0)).toBe(7_000)
    expect(nextBlinkDelay('reduced', () => 1)).toBe(12_000)
  })
