...
macOS/Linux 提供本地沙箱与安全文档预览（Linux 需安装 bubblewrap）。Windows 尚未完成运行时
适配，全量 CI 已发现落盘、权限语义和路径兼容性问题，不应视为与 macOS/Linux 等价的生产支持；
后台 Web 启动及安全文档预览/下载明确不支持。默认沙箱不可用时仍拒绝执行命令，不会自动
降为无沙箱模式。类型检查通过不等于跨平台运行时验收通过。
