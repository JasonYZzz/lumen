...
Web 运行时默认展开处理过程：主层展示进展和工具动作，原始思考、压缩与运行事件放在独立的按需展开详情中。只有连续、成功且声明可分组的工具活动会合并成摘要；失败与正在执行的工具逐条保留，所有并发中的工具均标记为执行中。已知内置搜索、网页阅读、文件读取与命令输出优先展示可读结果，原始输入与结果仍可切换查看；未知工具保持原始文本。截断和分页结果明确提示可用片段。主过程随对话滚动，原始详情和长工具结果使用有界阅读区域。用户手动展开或收起会跨完成状态保留；没有手动选择时成功结束默认收起，失败、中断默认展开，等待补充信息默认收起并保留独立澄清入口。新待审批动作会重新展开，但确认卡片独立于过程折叠，始终可操作。完成后的入口显示 Runtime 记录的实际耗时；没有计时数据的旧记录不显示时长。
