...
- `parallel_tool_calls` 决定 sequential / parallel_safe / parallel；
...
| 路径 | 公开事件 | 持久化结果 |
|---|---|---|
| 正常结束 | `RunCompleted` | 完整 outcome、messages、usage、plan |
| 阻塞澄清 | `RunWaitingForUser` | messages、pending question、`waiting_for_user` turn |
| 用户取消 | `RunCancelled` | partial text、已发生工具与审批 |
| provider/代码失败 | `RunFailed` | error、diagnostics、可重试标志 |
